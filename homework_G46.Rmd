...
# Define color mapping for activities
col_map <- c(
  "Sedentary" = "#2E86AB",   # Blue
  "Walking"   = "#A23B72",   # Magenta
  "Running"   = "#F18F01"    # Orange
)
```

## Data Preprocessing

Once obtained the data, first thing we do is transform the raw, asynchronously recorded sensor data, into a clean, synchronized, and evenly spaced dataset. By a quick look at the data, we noticed that the sensors, rarely recorded their read value at the exact same millisecond, so this pipeline forces all sensor readings onto a shared timeline so they can be accurately compared and analyzed row by row. In order to do this, we first order the raw data by timestamp and converts absolute time (milliseconds) into a relative, easy to read scale starting at exactly 0; established a universal 10Hz sampling rate, creating a target timeline with exactly one row every 0.1 seconds; we used the zoo package to calculate missing values. In this way, if a sensor didn't record a value at an exact 0.1s interval, the script mathematically estimates it by drawing a straight line between the nearest surrounding data points.
...
Finally, to rigorously identify any anomalies within those general trends, we check the functional boxplots. This allow us to check the functional mean (most typical movement curve for that activity) and distinctly highlights any abnormal, irregular strides as outliers in black.
...
mtext("FDA: 5-Second Acceleration Windows", side = 3, line = 0, outer = TRUE, font = 2, cex = 1.3)
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0)) # Reset layout
...
mtext("FDA Derivative Analysis: Jerk (1st Derivative)", side = 3, line = 0, outer = TRUE, font = 2, cex = 1.3)
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0)) # Reset layout
...
df_summary <- df_eval %>%
  ...
  mutate(Activity = factor(Activity, levels = c("Sedentary", "Walking", "Running")))
...
par(mfrow = c(1, 3), oma = c(0, 0, 2, 0)) # Set up 3-panel plot
for(act in c("Sedentary", "Walking", "Running")) {
  idx <- which(window_labels == act)
  if(length(idx) > 5) {
    # CORRECTED: fbplot expects numeric matrix, not fd object
...
mtext("Functional Boxplots by Activity", side = 3, line = 0, outer = TRUE, font = 2, cex = 1.3)
par(mfrow = c(1, 1), oma = c(0, 0, 0, 0)) # Reset plotting layout
```

## Conformal Prediction

Ok, now we transition from descriptive statistics into the actual Conformal Prediction framework in order to quantify uncertainty. The primary goal here is to build a rigorous "profile" (or an ellipsoidal confidence region) for each activity. If a new movement curve falls inside the sedentary profile but outside the running profile, we can confidently classify it. Here we don't guess the activity, but we provide a strict statistical guarantee (controlled by conf_alpha) about how likely the classification is correct. To construct these robust profiles, the first operation is Functional PCA (FPCA). This technique reduces complex 5-second curves into a few "Principal Components" (PCs). Instead of dealing with 50 distinct data points, a curve is now efficiently represented by just 2 or 3 numbers that successfully capture 90% of its overall shape. Once the curves are reduced to these core components, we apply the Mahalanobis Distance. This acts as a "smart" distance metric that accounts for how the data is stretched or rotated in multi-dimensional space. We use it to calculate a Conformity Score—essentially measuring how "weird" or "normal" a curve is relative to its assigned class. Using these conformity scores, we find a cutoff value ($q_{hat}$) such that $1 - \alpha$ (e.g., 90%) of our training data falls securely inside the boundary. With this threshold, we can generate our prediction bands. This process converts the abstract PC thresholds back into the physical time domain, creating a visual "envelope" or "tube." If a future movement curve stays completely within this tube, it is statistically guaranteed to be consistent with that specific activity. Finally, to ensure our theoretical guarantees hold up in practice, we check if 90% of our actual "Walking" data falls within the constructed "Walking" band and verify if "Running" data correctly falls outside that very same "Walking" band.
...
for(act in activities) {
  ...
  if (length(idx_other_global) > 0) {
    ...
    d2_other <- mahalanobis(scores_other, center = mu_scores, cov = Sigma_scores)
...
## CONFORMAL VISUALIZATION

This final section uses ggplot2 to visualize the boundaries of "normal" behavior for each activity. It generates two major plots that serve as the visual proof of your Conformal Prediction model.

Key Operations:

Plot 5.1 (Time-Domain Conformal Bands): This translates the ellipsoidal math back into the physical world. It plots the 90% confidence "tube" (ribbon) over time for each activity. If a new 5-second window of movement is recorded, and its curve fits completely inside the "Walking" tube, the model classifies it as walking.

Plot 5.2 (Global Latent Space Ellipsoids): Instead of looking at curves over time, this plot zooms out and looks at the mathematical separation of the activities. It runs a global Functional PCA to smash all curves into just two variables (PC1 and PC2). It then plots every single 5-second window as a dot, drawing 90% confidence boundaries (ellipses) around the clusters to see how well Sedentary, Walking, and Running separate from each other.
...
if (nrow(df_bands_all) == 0) {
  ...
  # Plot 5.2: Global Latent Space Conformal Ellipsoids
